    ...
    <div className="flex justify-center">
      <div className="flex flex-col w-full max-w-xl">
        <AddTodo
          ...
          initialValue={
            editId != null ? todos.find((todo) => todo.id === editId)?.text : ''
          }
            ...
            todos.map((todo) => (
              ...
              >
                <div className="flex items-center">
                  ...
                  <label
                    htmlFor={todo.id}
                    className={`text-white text-md font-medium ${todo.completed ? 'line-through' : ''}`}
                  >
                    {todo.text}
                  </label>
                </div>
      ...
      <div className="flex flex-col ml-8 w-1/3">
        ...
        {completedTodos.length > 0 ? (
          completedTodos.map((todo) => (
            <div
              key={todo.id}
              className="mt-4 bg-green-300 p-4 text-md font-medium rounded-lg"
            >
              {todo.text}
            </div>
          ))
        ) : (
          <div className="mt-4 bg-slate-100 p-4 text-gray-400 text-md font-medium rounded-lg">No completed todos.</div>
        )}
      </div>
    </div>
  );
